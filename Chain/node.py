...
class Node:
    ...
    def send_message_to_all(self, message):
        for peer in self.peers:
            try:
                ...
                print(f"Connection to {peer.host}:{peer.port} lost. Attempting to reconnect...")
                peer.reconnect()
    ...
    def receive_message(self, message):
        ...
        if message["seq_num"] not in self.requests:
            if message["message_type"] == "PrePrepare":
                ...
                self.pre_prepared[message["seq_num"]] = message
                ...
                if message["seq_num"] not in self.prepared:
                    self.prepared[message["seq_num"]] = []
                self.prepared[message["seq_num"]].append(processed_pre_prepare)
    ...
    def process_prepare(self, prepare):
        ...
        if prepare["seq_num"] not in self.prepared:
            self.prepared[prepare["seq_num"]] = []
        self.prepared[prepare["seq_num"]].append(prepare)
    ...
    def is_prepared(self, seq_num):
        if seq_num not in self.prepared:
            return False
    ...
    def process_commit(self, commit):
        ...
        if commit["seq_num"] not in self.prepared or commit["view_num"] != self.view_num:
            return
    ...
    def execute_operation(self, seq_num):
        ...
        self.blockchain.add_block(json.loads(pre_prepare["request"].replace("'", "\"")))
...
class PrimaryNode:
    ...
    def receive_request(self, request):
        if request not in self.node.requests:
